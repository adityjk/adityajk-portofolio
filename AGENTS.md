...
## 1. Project Overview
A single-page personal portfolio site (warm editorial minimal style — see DESIGN.md). Sections: hero, client logos, about, selected works list, recognitions, contact/footer. No CMS, no blog, no backend — static content, hand-edited data file for work items.
...
- Smooth scroll: **Lenis** (~4kb) — controls scroll momentum/easing globally, layered underneath Motion One (Motion One still handles individual element animations; Lenis only changes how scrolling itself feels). Loaded once, globally, on the base layout.
...
src/
  ...
  sections/         # one file per page section (Hero, About, SelectedWorks, Recognitions, Footer)
  data/
...
- Token names should map directly to DESIGN.md language, e.g.:
  ...
  - `--font-serif` (Fraunces), `--font-mono` (JetBrains Mono / Space Grotesk)
...
## 10. Deployment
- Hosting: **[FILL — matches DESIGN.md section 10, e.g. Vercel]**
- Deploy command: **[FILL — e.g. automatic on push to `main` via Vercel Git integration, or `bun run build` + manual upload]**
